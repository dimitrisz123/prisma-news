        ...
        if (newsValidation) {
          prisma.exists
            ...
            .then(resp => {
              const isValid = !resp;
              if (isValid) {
                addSport24ArticlesToDb(selector[i].attribs.href, rp, $, prisma);
              }
            })
        ...
        .createArticle(
          {
            data: {
              ...
              section: $("div > a.active", article)[0].children[0].data,
              summary: $("p.summary", article)[0].children[0].data,
              image: $("div.relatedPicture > img", article).attr("src"),
              content: null,
              time: new Date(
